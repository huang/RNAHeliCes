...
static double *MxMethodeHI (BarData *Data, double *);
...
/*==*/ 
static double*
MxMethodeHI (BarData *Data, double *Input)
{
  /***************************************/
  /*           |       E_s           |   */
  /*           \   ____________      /   */
  ...
  for( i = 0; i < dim; i++)
    for( j = i+1; j < dim; j++){
      m_saddle = Input[dim*i+j];  //max_saddle(i, j, Data);
      /* rate j -> i */
      U[dim*i+j] = D[dim*i+j]*exp(-(m_saddle-Data[j].energy)/_kT); 
      /* rate i -> j */
      U[dim*j+i] = D[dim*i+j]*exp(-(m_saddle-Data[i].energy)/_kT);
    }
  if(D != NULL) free(D);
  ...
  if (opt.want_verbose) MxPrint (U,"U with Methode H", 'm');
  free(Input);
  return (U);
}
