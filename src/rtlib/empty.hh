...
template<typename T> inline bool is_empty(const T &x)
{
  return x == 0;
}
...
//template<> inline bool is_empty(int x)
inline bool is_empty(char x)
{
  return x == '~' + 1;
}
...
//template<> inline bool is_empty(double x)
inline bool is_empty(double x)
{
  //assert( std::numeric_limits<double>::has_quiet_NaN );
  //return x != x;
  assert( std::numeric_limits<double>::has_infinity );
...
//template<> inline bool is_empty(const String &s)
...
inline bool is_empty(const Rope &s)
{
  return s.is_empty();
}
...
inline bool is_empty(const std::pair<T, U> &p)
{
  assert((is_empty(p.first) || is_empty(p.second))
         == (is_empty(p.first) && is_empty(p.second)));
  return is_empty(p.first);
}
...
template <typename T, typename U> inline bool is_empty(const Basic_Subsequence<T, U> &p)
{
  return p.is_empty();
}
...
template <typename X> inline bool is_empty(const rope::Ref<X> &p)
{
  return p.is_empty();
}
...
{
  return !(is_empty(x1) || is_empty(x2));
}
