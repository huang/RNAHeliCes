...
template<typename alphabet, typename pos_type, typename T>
inline bool regionpairing(const Basic_Sequence<alphabet, pos_type> &seq,
    T i, T j, int length)  // length = 2 is stackpairing
{
  bool pairing = true;
  for (int idx = 0; idx < length; idx++) {
    pairing = pairing && basepairing(seq, i+idx, j-idx);
  }
  return (i+3 < j) && pairing;
}
